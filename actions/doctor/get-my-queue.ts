...
export async function getMyQueueAction(): Promise<
  ActionResult<{ encounters: MyQueueItem[]; total: number }>
> {
  ...
  // Fetch encounters claimed by this doctor (WAIT_DOCTOR, IN_CONSULT, FOR_LAB, FOR_PHARMACY)
  const encounters = await db.encounter.findMany({
    where: {
      doctorId: session.userId,
      status: {
        in: ["WAIT_DOCTOR", "IN_CONSULT", "FOR_LAB", "FOR_PHARMACY"],
      },
  ...
  const items: MyQueueItem[] = encounters.map((encounter) => ({
    id: encounter.id,
    ...
    prescriptionCount: encounter._count.prescriptions,
  }))
